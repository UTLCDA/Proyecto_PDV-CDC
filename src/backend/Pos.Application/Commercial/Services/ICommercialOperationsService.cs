...
public interface ICommercialOperationsService
{
    ...
    Task<QuoteDto> CreateQuoteAsync(CreateQuoteDto request, Guid? currentUserId, string correlationId, CancellationToken cancellationToken = default);
    Task<SaleDto> ConvertQuoteToSaleAsync(Guid quoteId, Guid? currentUserId, string correlationId, string ipAddress, CancellationToken cancellationToken = default);
