...
public interface IInventoryApplicationService
{
    ...
    Task<List<InventoryMovementDto>> GetMovementsAsync(Guid? productId, string? movementType, string? search, DateTime? startDateUtc, DateTime? endDateUtc, CancellationToken cancellationToken = default, int page = 1, int pageSize = 500);
