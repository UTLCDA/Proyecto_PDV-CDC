...
public interface ISaleApplicationService
{
    ...
    Task<List<SaleDto>> GetSalesAsync(string? search, Guid? customerId, string? status, DateTime? startDate, DateTime? endDate, CancellationToken cancellationToken = default, int page = 1, int pageSize = 500);
