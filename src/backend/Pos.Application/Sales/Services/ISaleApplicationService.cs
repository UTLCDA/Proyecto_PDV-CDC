...
public interface ISaleApplicationService
{
    ...
    Task<PagedResult<SaleDto>> GetSalesAsync(string? search, Guid? customerId, string? status, DateTime? startDate, DateTime? endDate, CancellationToken cancellationToken = default, int page = 1, int pageSize = 25, string? sortBy = null, string? sortDirection = null);
