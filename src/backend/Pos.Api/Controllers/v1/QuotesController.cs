...
public class QuotesController : ControllerBase
{
    ...
    public QuotesController(ICommercialOperationsService commercialService)
    {
    ...
    public async Task<ActionResult<QuoteDto>> CreateQuote([FromBody] CreateQuoteDto request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var quote = await _commercialService.CreateQuoteAsync(request, currentUserId, correlationId, cancellationToken);
    ...
    public async Task<ActionResult<SaleDto>> ConvertQuoteToSale(Guid id, CancellationToken cancellationToken)
    {
        var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        Guid? currentUserId = Guid.TryParse(userIdClaim, out var userId) ? userId : null;
        var correlationId = HttpContext.Items["CorrelationId"]?.ToString() ?? Guid.NewGuid().ToString();
        ...
        try
        {
            var sale = await _commercialService.ConvertQuoteToSaleAsync(id, currentUserId, correlationId, ipAddress, cancellationToken);
            return Ok(sale);
        }
